...
const ListLeaveSetup = () => {
    ...
    const fetchLeaveSetups = useCallback(async () => {
        ...
        try {
            ...
            let sno = 1;
            const data = response.data.map((leaveSetup) => ({
                _id: leaveSetup._id,
                sno: sno++,
                leaveType: leaveSetup.leaveType,
                maxDays: leaveSetup.maxDays,
                description: leaveSetup.description || "N/A",
                action: <LeaveSetupButtons Id={leaveSetup._id} onLeaveSetupDelete={fetchLeaveSetups} />,
            }));
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(
                `http://localhost:5000/api/leavesetup/delete/${id}`,
                {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
                    }
